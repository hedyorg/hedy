...
def rule_all(level):
    ...
    # Rule for comments :
    list_rules.append(
        {"regex": "#.*$", "token": "comment", "next": "start", "unicode": True}
    )
    ...
    list_rules.append(
        {"regex": "_\\?_", "token": "invalid", "next": "start", "unicode": True}
    )
    list_rules.append(
        {
            "regex": "(^| )(_)(?= |$)",
    ...
    if data_level["number"]:
        ...
        list_rules.append(
            {
                "regex": START_WORD + number_regex + END_WORD,
                "token": ["text", "variable"],
                "next": "start",
                "unicode": True,
            }
        ...
        for command in data_level["space_before"]:
            list_rules.append(
                {
                    "regex": START_WORD
                    + get_translated_keyword(command)
                    + number_regex
                    + END_WORD,
        ...
        list_rules.append(
            {
                ...
                "token": [
                    "text",
                    "support.function"
                    if command in SECONDARY_COLOR_WORDS
                    else "keyword",
                ],
                "next": "start",
                "unicode": True,
