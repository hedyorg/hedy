...
def get_translations(KEYWORDS_PATH, KEYWORDS_PATTERN):
    ...
    for language_file in list_language_file:
        ...
        if m := re.search(KEYWORDS_PATTERN, language_file):
            ...
            tmp[language_code] = get_yaml_content(
                os.path.join(KEYWORDS_PATH, language_file)
            )
    ...
    for language_code in sorted(tmp.keys()):
        # KEYWORDS
        result[language_code] = get_commands(
            language_code, tmp[language_code], reference, TRANSLATE_WORDS
        )
