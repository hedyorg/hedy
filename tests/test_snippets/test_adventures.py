                    ...
                    for level_number in adventure["levels"]:
                        if level_number > hedy.HEDY_MAX_LEVEL:
                            ...
                            try:
                                ...
                                print(
                                    f"Problem reading startcode for {lang} level {level}"
                                )
                                pass
                            # Code snippets inside example code
                            try:
                                example_code = utils.markdown_to_html_tags(
                                    level["example_code"]
                                )
...
Hedy_snippets = [
    (s.name, s)
    for s in collect_snippets(
        path="../../content/adventures", filtered_language=filtered_language
    )
]
