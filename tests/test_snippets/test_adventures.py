        ...
        if snippet is not None and len(snippet.code) > 0:
            try:
                ...
                if fix_error:
                    ...
                    original_text = original_yaml['adventures'][snippet.key]['levels'] \
                        [snippet.level][snippet.field_name]

                    # Read broken yaml file
                    broken_yaml = utils.load_yaml_rt(snippet.filename)
                    broken_yaml['adventures'][snippet.key]['levels'] \
                        [snippet.level][snippet.field_name] = original_text
