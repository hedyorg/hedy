...
Hedy_snippets = [
    (s.name, s) for s in collect_snippets(path="../../content/cheatsheets")
]
...
if lang:
    Hedy_snippets = [
        (name, snippet)
        for (name, snippet) in Hedy_snippets
        if snippet.language[:2] == lang
    ]
                ...
                with app.app_context():
                    with force_locale("en"):
                        ...
                        error_message = error_message.replace(
                            '<span class="command-highlighted">', "`"
                        )
                        error_message = error_message.replace("</span>", "`")
                        print(f"\n----\n{snippet.code}\n----")
                        print(
                            f"in language {snippet.language} from level {snippet.level} gives error:"
                        )
