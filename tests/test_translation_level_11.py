...
class TestsTranslationLevel11(HedyTester):
    ...
    def test_for_in_english_dutch(self):
        code = textwrap.dedent("for counter in range 1 to 5\n"
                               "    print counter")

        result = hedy_translation.translate_keywords(code, from_lang="en", to_lang="nl", level=self.level)
        expected = textwrap.dedent("voor counter in bereik 1 tot 5\n"
                                   "    print counter")
    ...
    def test_for_in_dutch_english(self):
        code = textwrap.dedent("nummer is vraag 'hoe oud ben je'\n"
                               "voor counter in bereik 1 tot 5\n"
                               "    voor count in bereik nummer tot 0\n"
                               "        print 'hoi' counter")

        result = hedy_translation.translate_keywords(code, from_lang="nl", to_lang="en", level=self.level)
        expected = textwrap.dedent("nummer is ask 'hoe oud ben je'\n"
