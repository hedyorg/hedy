...
class TestDynamoAbstraction(unittest.TestCase, Helpers):
    ...
    def test_batch_get(self):
        self.insert(dict(id="k1", bla=1), dict(id="k2", bla=2), dict(id="k3", bla=3))
...
class TestSortKeysInMemory(unittest.TestCase):
    ...
    def setUp(self):
        self.table = dynamo.Table(
            dynamo.MemoryStorage(), "table", partition_key="id", sort_key="sort"
        )
...
class TestQueryInMemory(unittest.TestCase, Helpers):
    ...
    def test_get_many_iterator(self):
        ...
        for i in range(N):
            self.insert(dict(id="key", sort=i + 1))

        expected = [dict(id="key", sort=i + 1) for i in range(N)]

        # Query everything at once, using the Python iterator protocol
        self.assertEqual(
            list(dynamo.GetManyIterator(self.table, dict(id="key"))), expected
        )

        # Query paginated, using the Python iterator protocol
        self.assertEqual(
            list(dynamo.GetManyIterator(self.table, dict(id="key"), limit=3)), expected
        )
        ...
        token = None
        while True:
            many = dynamo.GetManyIterator(
                self.table, dict(id="key"), limit=3, pagination_token=token
            )
