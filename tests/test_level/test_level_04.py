...
class TestsLevel4(HedyTester):
    ...
    def test_print_with_hashtag(self):
        code = "print 'Hello #world'"
        expected = "print(f'Hello #world')"

        self.multi_level_tester(code=code, max_level=11, expected=expected)

    def test_print_another_hashtag(self):
        code = "print '#Helloworld'"
        expected = "print(f'#Helloworld')"
    ...
    def test_print_with_space_gives_invalid(self):
        ...
        skipped_mappings = [
            SkippedMapping(
                SourceRange(1, 1, 1, 32), hedy.exceptions.InvalidSpaceException
            ),
            SkippedMapping(
                SourceRange(2, 1, 2, 15), hedy.exceptions.InvalidCommandException
            ),
        ]
    ...
    def test_print_backslash(self):
        ...
        self.multi_level_tester(
            code=code, expected=expected, output=output, max_level=11, translate=True
        )
    ...
    def test_print_with_backslash_at_end(self):
        ...
        self.multi_level_tester(
            code=code, max_level=11, expected=expected, translate=True
        )
    ...
    def test_print_without_quotes_gives_error_from_grammar(self):
        ...
        skipped_mappings = [
            SkippedMapping(
                SourceRange(1, 1, 1, 15), hedy.exceptions.UnquotedTextException
            ),
            SkippedMapping(
                SourceRange(2, 1, 2, 15), hedy.exceptions.InvalidCommandException
            ),
    ...
    def test_ask_without_quotes_gives_error_from_grammar(self):
        ...
        skipped_mappings = [
            SkippedMapping(
                SourceRange(1, 1, 1, 23), hedy.exceptions.UnquotedTextException
            ),
            SkippedMapping(
                SourceRange(2, 1, 2, 15), hedy.exceptions.InvalidCommandException
            ),
    ...
    def test_one_mistake_not_skipped(self):
        ...
        self.multi_level_tester(
            code=code, exception=hedy.exceptions.InvalidCommandException, max_level=17
        )
    ...
    def test_print_without_opening_quote_gives_error(self, q):
        ...
        skipped_mappings = [
            SkippedMapping(
                SourceRange(1, 1, 1, 16), hedy.exceptions.UnquotedTextException
            ),
            SkippedMapping(
                SourceRange(2, 1, 2, 15), hedy.exceptions.InvalidCommandException
            ),
        ...
        self.multi_level_tester(
            code, expected=expected, skipped_mappings=skipped_mappings, max_level=5
        )
    ...
    def test_print_quoted_var_reference(self):
        ...
        self.multi_level_tester(
            code=code, expected=expected, max_level=11, unused_allowed=True
        )
    ...
    def test_print_comment(self):
        ...
        self.multi_level_tester(
            max_level=11, code=code, expected=expected, output=output
        )
    ...
    def test_assign_comment(self):
        ...
        self.multi_level_tester(
            max_level=11, unused_allowed=True, code=code, expected=expected
        )
    ...
    def test_ask_without_quotes_gives_error(self):
        ...
        skipped_mappings = [
            SkippedMapping(
                SourceRange(1, 1, 1, 22), hedy.exceptions.UnquotedTextException
            ),
            SkippedMapping(
                SourceRange(2, 1, 2, 15), hedy.exceptions.InvalidCommandException
            ),
    ...
    def test_print_list_access_index_var(self):
        ...
        def check_in_list(x): return HedyTester.run_code(x) == "Hond"
    ...
    def test_sleep_with_input_variable(self):
        ...
        expected = HedyTester.dedent(
            "n = input(f'how long')", HedyTester.sleep_command_transpiled("n")
        )
    ...
    def test_var_undefined_error_message(self):
        ...
        self.multi_level_tester(
            code=code, max_level=11, exception=hedy.exceptions.UndefinedVarException
        )
    ...
    def test_program_gives_hedy_parse_exception(self):
        ...
        self.multi_level_tester(
            ...
            and c.exception.error_location[1] == 1,
        )
    ...
    def test_quoted_text_gives_error(self):
        ...
        skipped_mappings = [
            SkippedMapping(
                SourceRange(1, 1, 1, 30), hedy.exceptions.MissingCommandException
            ),
            SkippedMapping(
                SourceRange(2, 1, 2, 15), hedy.exceptions.InvalidCommandException
            ),
    ...
    def test_lonely_text(self):
        ...
        skipped_mappings = [
            SkippedMapping(
                SourceRange(1, 1, 1, 8), hedy.exceptions.LonelyTextException
            ),
            SkippedMapping(
                SourceRange(2, 1, 2, 15), hedy.exceptions.InvalidCommandException
            ),
