...
class HedyTester(unittest.TestCase):
    ...
    def tearDownClass(cls):
        ...
        if os.getenv("save_snippet_hashes") and cls.snippet_hashes_original_len != len(
            cls.snippet_hashes
        ):
    ...
    def snippet_already_tested_with_current_hedy_version(self, snippet, level):
        try:
            hash_language_plus_snippet_and_level = self.create_hash(
                self.all_language_texts, snippet, level
            )
            return hash_language_plus_snippet_and_level in self.snippet_hashes
        except (
            UnicodeEncodeError
        ):  # some tests (generated by Hypothesis) can't be hashed
            return False
    ...
    def single_level_tester(
        ...
        if (
            level is None
        ):  # no level set (from the multi-tester)? grap current level from class
            level = self.level
        if not self.snippet_already_tested_with_current_hedy_version(code, level):
            if skipped_mappings is not None:
                ...
                for skipped in skipped_mappings:
                    result_error = result.source_map.get_error_from_hedy_source_range(
                        skipped.source_range
                    )
                ...
                if exception is not None:
                    ...
                    if (
                        ("ask" not in all_commands)
                        and ("input" not in all_commands)
                        and ("clear" not in all_commands)
                    ):
                        self.assertTrue(self.validate_Python_code(result))
                    if output is not None:
                        if (
                            extra_check_function is None
                        ):  # most programs have no turtle so make that the default
                            extra_check_function = self.is_not_turtle()
                        self.assertEqual(output, HedyTester.run_code(result))
                        self.assertTrue(extra_check_function(result))

            # all ok? -> save hash!
            self.snippet_hashes.add(
                self.create_hash(self.all_language_texts, code, level)
            )

    def source_map_tester(self, code, expected_source_map: dict):
        result = hedy.transpile(code, self.level, "en")
        self.assertDictEqual(
            result.source_map.get_compressed_mapping(), expected_source_map
        )
    ...
    def validate_Python_code(parseresult):
        ...
        try:
            if (
                not parseresult.has_turtle and not parseresult.has_pygame
            ):  # ouput from turtle or pygame cannot be captured
    ...
    def dedent(*args):
        return "\n".join(
            [
                textwrap.indent(textwrap.dedent(a[0]), a[1])
                if isinstance(a, tuple)
                else textwrap.dedent(a)
                for a in args
            ]
        )
        ...
        for snippet in snippets:
            try:
                if snippet[1].language in ALL_KEYWORD_LANGUAGES.keys():
                    snippet[1].code = snippet[1].code.format(
                        **keyword_dict[snippet[1].language]
                    )
                else:
                ...
                print(
                    "This following snippet contains an unclosed invalid placeholder..."
                )
                print(snippet)
