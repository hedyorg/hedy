...
class WebsiteModule(flask.Blueprint):
    ...
    the routes can also share private variables that are injected from 'app.py', for example the database. The alternative (using a global g.db) would be untyped and then the IDE lacks information about the type causing less autocomplete support.
