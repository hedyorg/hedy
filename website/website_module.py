...
class WebsiteModule(flask.Blueprint):
    ...
    The alternative (using a global g.db) would be untyped and then the IDE lacks information about the type,
    causing less autocomplete support.
...
def route(*args, **kwargs):
    def wrap(fn):
        routing_data = getattr(fn, '_routing_data', [])
        routing_data.append(RoutingData(args, kwargs))
        setattr(fn, '_routing_data', routing_data)
        return fn
    return wrap
