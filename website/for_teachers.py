...
class ForTeachersModule(WebsiteModule):
    ...
    def get_class(self, user, class_id):
        ...
        for student in students:
            student["last_login"] = utils.localized_date_format(
                student.get("last_login", 0)
            )
            ...
            invites.append(
                {
                    ...
                    "timestamp": utils.localized_date_format(
                        invite["timestamp"], short_format=True
                    ),
                    "expire_timestamp": utils.localized_date_format(
                        invite["ttl"], short_format=True
                    ),
                }
            )
    ...
    def get_class_customization_page(self, user, class_id):
        ...
        return render_template(
            ...
            min_level=min_level,
            class_id=class_id,
            javascript_page_options=dict(page="customize-class", class_id=class_id),
        )
    ...
    def add_adventure(self, user, level):
        ...
        is_teacher_adventure = self.is_adventure_from_teacher(
            adventure_id, teacher_adventures
        )
    ...
    def sort_adventures_in_class(self, user):
        ...
        for adventure in adventures_from_request:
            is_teacher_adventure = self.is_adventure_from_teacher(
                adventure, teacher_adventures
            )
            customizations["sorted_adventures"][level].append(
                {"name": adventure, "from_teacher": is_teacher_adventure}
            )
            sorted_adventure = SortedAdventure(
    ...
    def get_class_info(self, user, class_id):
        if hedy_content.Adventures(g.lang).has_adventures():
            default_adventures = hedy_content.Adventures(
                g.lang
        ...
        if customizations:
            ...
            if "sorted_adventures" in customizations:
                ...
                self.purge_customizations(
                    customizations["sorted_adventures"], default_adventures
                )
            # it uses the old way so convert it to the new one
            elif "adventures" in customizations:
                customizations["sorted_adventures"] = {
            ...
            self.db.update_class_customizations(customizations)
            min_level = (
                1 if customizations["levels"] == [] else min(customizations["levels"])
            )
        else:
            ...
            for (
                level,
                default_adventures,
            ) in hedy_content.ADVENTURE_ORDER_PER_LEVEL.items():
                adventures_to_db[str(level)] = [
                    {"name": adventure, "from_teacher": False}
    ...
    def purge_customizations(self, sorted_adventures, adventures):
        for _, adventure_list in sorted_adventures.items():
            for adventure in list(adventure_list):
                if (
                    not adventure["from_teacher"]
                    and adventure["name"] not in adventures
                ):
    ...
    def get_unused_adventures(self, adventures, teacher_adventures_db, adventure_names):
        ...
        for (
            level,
            level_default_adventures,
        ) in hedy_content.ADVENTURE_ORDER_PER_LEVEL.items():
            for short_name in level_default_adventures:
                adventure = SortedAdventure(
        ...
        for teacher_adventure in teacher_adventures_db:
            adventure = SortedAdventure(
                short_name=teacher_adventure["id"],
        ...
        for level in range(1, hedy.HEDY_MAX_LEVEL + 1):
            ...
            available_teacher_adventures = teacher_adventures[level].difference(
                adventures_set
            )
            available_default_adventures = default_adventures[level].difference(
                adventures_set
            )
            available_adventures[level] = list(
                available_teacher_adventures.union(available_default_adventures)
            )
    ...
    def update_customizations(self, user, class_id):
        ...
        achievement = self.achievements.add_single_achievement(
            user["username"], "my_class_my_rules"
        )
        if achievement:
            return {
                "achievement": achievement,
                "success": gettext("class_customize_success"),
            }, 200
    ...
    def store_accounts(self, user):
        ...
        for account in body.get("accounts", []):
            ...
            if account.get("username").strip().lower() in usernames:
                return {
                    "error": gettext("unique_usernames"),
        ...
        for account in body.get("accounts", []):
            if account.get("class") and account["class"] not in [
                i.get("name") for i in classes
            ]:
                return "not your class", 404
            if self.db.user_by_username(account.get("username").strip().lower()):
                return {
                    "error": gettext("usernames_exist"),
                    "value": account.get("username").strip().lower(),
                }, 200
        ...
        for account in body.get("accounts", []):
            ...
            if account.get("class"):
                class_id = [
                    i.get("id")
                    for i in classes
                    if i.get("name") == account.get("class")
                ][0]
                self.db.add_student_to_class(
                    class_id, account.get("username").strip().lower()
                )
    ...
    def view_adventure(self, user, adventure_id):
        if not is_teacher(user) and not is_admin(user):
            return utils.error_page(
                error=403, ui_message=gettext("retrieve_adventure_error")
            )
        ...
        adventure["content"] = adventure["content"].replace(
            "<pre>",
            "<pre class='no-copy-button' level='" + str(adventure["level"]) + "'>",
        )
