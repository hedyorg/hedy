        ...
        # /session_test is meant to return the session from the test environment, for testing purposes.
            ...
            for header in r.headers:
                ...
                if header.lower() in ['content-encoding', 'content-length', 'transfer-encoding', 'connection']:
                    continue
                # Setting the session cookie returned by the test environment into the
                # response won't work because it will be overwritten by Flask, so we need
                # to read the cookie into the session so that then the session cookie can
                # be updated by Flask
...
def redirect_ab(request):
    ...
    # This will send either % PROXY_TO_TEST_PROPORTION of the requests into
    # redirect, or 50% if that variable is not specified.
