...
def request(method, path, headers={}, body="", cookies=None):
    ...
    response = getattr(requests, method)(
        HOST + path, headers=headers, data=body, cookies=cookies
    )
...
class AuthHelper(unittest.TestCase):
    ...
    def assert_user_exists(self, username):
        if not isinstance(username, str):
            raise Exception(
                "AuthHelper.assert_user_exists - Invalid username: " + str(username)
            )
        ...
        response = request(
            "post", "auth/signup", {}, body, cookies=self.user_cookies[username]
        )
    ...
    def make_current_user_teacher(self):
        ...
        """
        self.post_data(
            "admin/markAsTeacher", {"username": self.username, "is_teacher": True}
        )
    ...
    def post_data(
        ...
        cookies = (
            self.user_cookies[self.username]
            if self.username and not no_cookie
            else None
        )
    ...
    def delete_data(
        self, path, expect_http_code=200, no_cookie=False, return_headers=False
    ):
        cookies = (
            self.user_cookies[self.username]
            if self.username and not no_cookie
...
class TestSessionVariables(AuthHelper):
    def test_get_session_variables(self):
        ...
        self.assertEqual(
            body["session"]["test_session"], test_body["session"]["session_id"]
        )
...
class TestAuth(AuthHelper):
    ...
    def test_verify_email(self):
        ...
        headers = self.get_data(
            ...
            + urllib.parse.urlencode(
                {"username": self.username, "token": self.user["verify_token"]}
            ),
    ...
    def test_change_password(self):
        ...
        # THEN receive an OK response code from the server
        self.post_data(
            "auth/login", {"username": self.username, "password": new_password}
        )
    ...
    def test_invalid_recover_password(self):
        ...
        # THEN receive a forbidden response code from the server
        self.post_data(
            "auth/recover", {"username": self.make_username()}, expect_http_code=403
        )
    ...
    def test_reset_password(self):
        ...
        new_password = "pas1234"
        recover_token = self.post_data("auth/recover", {"username": self.username})[
            "token"
        ]
        # THEN receive an OK response code from the server
        self.post_data(
...
class TestProgram(AuthHelper):
    ...
    def test_create_program(self):
        ...
        for key in program:
            ...
            if key not in keys_to_ignore:
                self.assertEqual(
                    program.get(key, None),
                    saved_program.get(key, None),
                    f"Difference on key {key}",
                )
    ...
    def test_invalid_make_program_public(self):
        ...
        self.post_data(
            "programs/share", {"id": "123456", "public": True}, expect_http_code=404
        )
...
class TestClasses(AuthHelper):
    ...
    def test_invalid_update_class(self):
        ...
        self.post_data(
            "class/foo", {"name": "class2"}, expect_http_code=404, put_data=True
        )
    ...
    def test_join_class(self):
        ...
        # THEN receive a forbidden status code from the server
        self.post_data(
            "class/join", {"id": Class["id"]}, no_cookie=True, expect_http_code=403
        )
        ...
        if not re.search(
            HOST + "class/" + Class["id"] + "/prejoin/" + Class["link"], body
        ):
...
class TestCustomizeClasses(AuthHelper):
    def test_not_allowed_customization(self):
        ...
        # THEN receive a forbidden response code from the server
        self.post_data(
            "for-teachers/customize-class/" + class_id, {}, expect_http_code=403
        )
    ...
    def test_invalid_customization(self):
        ...
        # THEN receive a not found response code from the server
        self.post_data(
            "for-teachers/customize-class/123" + class_id, {}, expect_http_code=404
        )
    ...
    def test_remove_customization(self):
        ...
        # THEN receive an OK response code with the server
        self.post_data(
            "for-teachers/restore-customizations?level=1", {}, expect_http_code=200
        )
...
class TestCustomAdventures(AuthHelper):
    ...
    def test_invalid_create_adventure(self):
        ...
        for invalid_body in invalid_bodies:
            self.post_data(
                "for-teachers/create_adventure", invalid_body, expect_http_code=400
            )
    ...
    def test_invalid_update_adventure(self):
        ...
        for invalid_body in invalid_bodies:
            self.post_data(
                "for-teachers/customize-adventure", invalid_body, expect_http_code=400
            )
...
class TestMultipleAccounts(AuthHelper):
    ...
    def test_invalid_create_accounts(self):
        ...
        for invalid_body in invalid_bodies:
            self.post_data(
                "for-teachers/create-accounts", invalid_body, expect_http_code=400
            )
