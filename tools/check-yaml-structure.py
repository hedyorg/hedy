...
def main():
    ...
    for reference_file in glob.glob("content/*/en.yaml"):
        ...
        mismatches = {
            comparison_file: find_mismatched_arrays(en, load_yaml(comparison_file))
        ...
        if mismatches:
            ...
            print(
                f"==================== {path.dirname(reference_file)} ======================="
            )
            print("    Different array lengths between English and other languages.")
            print(
                "    Please make the arrays the same by copying the new English content"
            )
            ...
            unique_paths = list(
                sorted(set(p for paths in mismatches.values() for p in paths))
            )
            for p in unique_paths:
                mis_by_file = {
                    lang_file: mis_by_path[p]
                    for lang_file, mis_by_path in mismatches.items()
                    if mis_by_path.get(p)
                }
