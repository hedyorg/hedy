...
def adventures_list():
    adventures = load_adventure_for_language(g.lang)
    return render_template('adventures.html', adventures=adventures, auth=TRANSLATIONS.get_translations(g.lang, 'Auth'))
...
def adventure_page(adventure_name, level):
    ...
    if not adventure_name in adventures:
        return utils.page_404 (TRANSLATIONS, current_user()['username'], g.lang, TRANSLATIONS.get_translations (g.lang, 'ui').get ('no_such_adventure'))
    ...
    if not level in adventure['levels']:
        return utils.page_404 (TRANSLATIONS, current_user()['username'], g.lang, TRANSLATIONS.get_translations (g.lang, 'ui').get ('no_such_adventure_level'))
