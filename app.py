...
from website.flask_helpers import (
    render_template,
    proper_tojson,
    JinjaCompatibleJsonProvider,
)
...
def load_adventures_for_level(level):
    ...
    all_adventures.sort(
        key=lambda pair: index_map.get(pair["short_name"], len(adventures_order))
    )

    return all_adventures


def load_saved_programs(
    level, into_adventures, preferential_program: Optional[Program]
):
    ...
    loaded_programs = {
        k: Program.from_database_row(r)
        for k, r in DATABASE.last_level_programs_for_user(
...
def load_customized_adventures(level, customizations, into_adventures):
    ...
    for a in order_for_this_level:
        if a["from_teacher"] and (db_row := teacher_adventure_map.get(a["name"])):
            try:
                db_row["content"] = safe_format(
                    db_row["content"], **hedy_content.KEYWORDS.get(g.keyword_lang)
                )
            except Exception:
                # We don't want teacher being able to break the student UI -> pass this adventure
                pass

            into_adventures.append(
                Adventure.from_teacher_adventure_database_row(db_row)
            )
...
def before_request_begin_logging():
    ...
    """
    path = (
        (str(request.path) + "?" + request.query_string.decode("utf-8"))
...
def setup_language():
    ...
    if "keyword_lang" not in session:
        session["keyword_lang"] = (
            g.lang if g.lang in ALL_KEYWORD_LANGUAGES.keys() else "en"
        )
...
def add_generated_css_file():
    return {
        "generated_css_file": "/css/generated.full.css"
        if is_debug_mode()
        else "/css/generated.css"
    }
...
def parse():
    ...
    querylog.log_value(
        level=level, lang=lang, session_id=utils.session_id(), username=username
    )
    ...
    try:
        ...
        with querylog.log_time("transpile"):
            try:
                ...
                translated_error = translate_error(
                    ex.error_code, ex.arguments, keyword_lang
                )
        ...
        try:
            if (
                ...
                and ACHIEVEMENTS.verify_run_achievements(
    ...
    if username and body.get("save_name"):
        try:
            ...
            response["save_info"] = SaveInfo.from_program(
                Program.from_database_row(program)
            )
...
def parse_by_id(user):
    ...
    if program and program.get("username") == user["username"]:
        try:
            hedy.transpile(
                program.get("code"), program.get("level"), program.get("lang")
            )
            return {}, 200
        except BaseException:
...
def transpile_add_stats(code, level, lang_, skip_faulty):
    ...
    try:
        ...
        statistics.add(
            username,
            lambda id_: DATABASE.add_program_stats(id_, level, number_of_lines, None),
        )
        return result
        ...
        statistics.add(
            username,
            lambda id_: DATABASE.add_program_stats(
...
def translate_error(code, arguments, keyword_lang):
    ...
    if "tip" in arguments:
        error_template = error_template.replace(
            "{tip}", gettext("" + str(arguments["tip"]))
        )
    ...
    for k, v in arguments.items():
        ...
        if k in arguments_that_require_highlighting:
            if k in arguments_that_require_translation:
                local_keyword = hedy_translation.translate_keyword_from_en(
                    v, keyword_lang
                )
...
def all_commands(id):
    ...
    return render_template(
        "commands.html", commands=hedy.all_commands(code, level, lang)
    )
...
def achievements_page():
    ...
    achievements = ACHIEVEMENTS_TRANSLATIONS.get_translations(g.lang).get(
        "achievements"
    )
...
def programs_page(user):
    ...
    next_page_url = (
        url_for("programs_page", **dict(request.args, page=result.next_page_token))
...
def query_logs():
    ...
    if not is_admin(user):
        ...
        if (
            not class_
            or class_["teacher"] != user["username"]
...
@app.route(
    "/onlinemasters", methods=["GET"], defaults={"level": "1", "program_id": None}
)
...
def index(level, program_id):
    ...
    if "levels" in customizations:
        ...
        for current_level, timestamp in customizations.get("opening_dates", {}).items():
            if utils.datetotimeordate(timestamp) > utils.datetotimeordate(
                utils.mstoisostring(now)
            ):
                try:
    ...
    if "level_thresholds" in customizations:
        if "quiz" in customizations.get("level_thresholds"):
            ...
            if level < hedy.HEDY_MAX_LEVEL and QUIZZES[g.lang].get_quiz_data_for_level(
                level
            ):
                scores = [
                    x.get("scores", []) for x in quiz_stats if x.get("level") == level
                ]
                scores = [score for week_scores in scores for score in week_scores]
                max_score = 0 if len(scores) < 1 else max(scores)
                ...
                if max_score < threshold:
                    ...
                    available_levels = available_levels[
                        : available_levels.index(level) + 1
                    ]
        ...
        if not any(a.short_name == loaded_program.adventure_name for a in adventures):
            adventures.append(
                Adventure(
                    ...
                    is_teacher_adventure=False,
                    is_command_adventure=loaded_program.adventure_name
                    in KEYWORDS_ADVENTURES,
                )
            )
...
def view_program(user, id):
    ...
    result["public_profile"] = (
        True if DATABASE.get_public_profile_settings(result["username"]) else None
    )

    code = result["code"]
    if (
        result.get("lang") != "en"
        and result.get("lang") in ALL_KEYWORD_LANGUAGES.keys()
    ):
    ...
    arguments_dict["initial_adventure"] = dict(
        result,
        start_code=code,
    )
    ...
    return render_template(
        "view-program-page.html",
        blur_button_available=True,
        javascript_page_options=dict(
            page="view-program", lang=g.lang, level=int(result["level"])
        ),
        **arguments_dict,
    )
...
def get_certificate_page(username):
    ...
    return render_template(
        "printable/certificate.html",
        count_programs=count_programs,
        quiz_score=quiz_score,
        longest_program=longest_program,
        number_achievements=number_achievements,
...
def get_highest_quiz_level(username):
    ...
    return (
        max([x.get("level") for x in quiz_scores if x.get("finished")])
        if finished_quizzes
        else "-"
    )
...
def signup_page():
    ...
    return render_template(
        "signup.html", page_title=gettext("title_signup"), current_page="login"
    )
...
def login_page():
    if current_user()["username"]:
        return redirect("/my-profile")
    return render_template(
        "login.html", page_title=gettext("title_login"), current_page="login"
    )
...
def profile_page(user):
    ...
    public_profile_settings = DATABASE.get_public_profile_settings(
        current_user()["username"]
    )
...
def main_page():
    sections = hedyweb.PageTranslations("start").get_page_translations(g.lang)[
        "start-sections"
    ]
...
@app.route("/privacy")
def privacy():
    privacy_translations = hedyweb.PageTranslations("privacy").get_page_translations(
        g.lang
    )
    return render_template(
        "privacy.html",
...
def explore():
    ...
    programs = normalize_public_programs(
        DATABASE.get_public_programs(
            limit=40,
            level_filter=level,
...
def get_highscores_page(user, filter):
    ...
    public_profile = (
        True if DATABASE.get_public_profile_settings(user["username"]) else False
    )
    ...
    for highscore in highscores:
        highscore["country"] = (
            highscore.get("country") if highscore.get("country") else "-"
        )
...
def get_slides(level):
    ...
    keyword_language = request.args.get(
        "keyword_language", default=g.keyword_lang, type=str
    )
...
def get_tutorial_translation(level, step):
    ...
    if step == "code_snippet":
        code = hedy_content.deep_translate_keywords(
            gettext("tutorial_code_snippet"), g.keyword_lang
        )
...
def other_languages():
    ...
    current_lang = g.lang
    return [
        make_lang_obj(lang) for lang in ALL_LANGUAGES.keys() if lang != current_lang
    ]
...
def update_public_profile(user):
    ...
    if not isinstance(body.get("image"), str) or int(body.get("image"), 0) not in [
        *range(1, 13)
    ]:
        return gettext("image_invalid"), 400
    if not isinstance(body.get("personal_text"), str):
        return gettext("personal_text_invalid"), 400
    if "favourite_program" in body and not isinstance(
        body.get("favourite_program"), str
    ):
    ...
    if "favourite_program" in body:
        ...
        if (
            not program
            or program.get("username") != user["username"]
            or not program.get("public")
        ):
            return gettext("favourite_program_invalid"), 400
...
def public_user_page(username):
    ...
    page = request.args.get("page", default=None, type=str)
    if user_public_info:
        user_programs = DATABASE.public_programs_for_user(
            username, limit=10, pagination_token=page
        )
        ...
        if (
            "favourite_program" in user_public_info
            and user_public_info["favourite_program"]
        ):
        ...
        next_page_url = (
            url_for(
                "public_user_page",
                username=username,
                **dict(request.args, page=next_page_token),
            )
            if next_page_token
            else None
        )
...
def teacher_invitation(code):
    ...
    if not valid_invite_code(code):
        return utils.error_page(
            error=404, ui_message=gettext("invalid_teacher_invitation_code")
        )
...
def current_user_allowed_to_see_program(program):
    ...
    if is_teacher(user) and program["username"] in DATABASE.get_teacher_students(
        user["username"]
    ):
        return True
...
if __name__ == "__main__":
    ...
    app.run(
        threaded=True, debug=not is_in_debugger, port=config["port"], host="0.0.0.0"
    )
