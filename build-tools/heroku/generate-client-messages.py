...
def main():
    ...
    for lang, trans in translations.items():
        try:
            translator = gettext.translation(
                "messages", localedir="translations", languages=[lang, "en"]
            )
    ...
    with open(OUTPUT_FILE, "w", encoding="utf-8") as f:
        ...
        f.write(
            ...
            + " | ".join(f"'{k}'" for k in sorted(message_keys))
            + "\n"
        )
        f.write(
...
def validate_gettext_hackfile():
    ...
    """
    filename = path.join(
        path.dirname(__file__), "..", "..", "content", "client-messages.txt"
    )
    ...
    for additional_key in ADDITIONAL_GETTEXT_KEYS:
        ...
        if gettext_line not in messages:
            oopsie = True
            print(
                f"Put the following line into content/client-messages.txt:   {gettext_line}"
            )
