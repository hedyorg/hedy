...
export const addIncorrectLineEffect = StateEffect.define<{
  from: number;
  to: number;
}>({
  map: (val, maping) => ({
    from: maping.mapPos(val.from),
    to: maping.mapPos(val.to),
  }),
});
  ...
  update(errors, transaction) {
    ...
    for (let e of transaction.effects) {
      if (e.is(addErrorLine)) {
        ...
        const line = transaction.state.doc.line(e.value.row);
        const length = line.text
          .slice(e.value.col - 1)
          .split(/(\s+)/)[0].length;
        if (length > 0) {
          errors = errors.update({
            add: [
              errorHighlightMark.range(
                line.from + e.value.col - 1,
                line.from + e.value.col - 1 + length
              ),
            ],
          });
        } else {
          // If we can't find the word, highlight the whole line
          errors = errors.update({
            add: [errorHighlightLine.range(line.from, line.from)],
          });
        }
  ...
  update(set, transaction) {
    ...
    for (let e of transaction.effects) {
      if (e.is(breakpointGutterEffect)) {
        if (e.value.on)
          set = set.update({
            add: [deactivateGutterMarker.range(e.value.pos)],
          });
...
class PlacheholderWidget extends WidgetType {
  constructor(readonly space: string, readonly placeholder: string) {
    super();
  }
...
function highlightVariables(view: EditorView) {
  ...
  let definingCommands = ["Assign", "Ask", "AssignList", "For", "Input"];
              ...
              for (const name of varNames) {
                if (variablesNames.has(name)) {
                  ...
                  variableData.push({
                    ...
                    pos: {
                      from: child.from + index,
                      to: child.from + index + name.length,
                    },
                  });
...
function getVarNames(name: string) {
  const varRegex =
    /^[\p{Lu}\p{Ll}\p{Lt}\p{Lm}\p{Lo}\p{Nl}_]+([\p{Lu}\p{Ll}\p{Lt}\p{Lm}\p{Lo}\p{Nl}_]+|[\p{Mn}\p{Mc}\p{Nd}\p{Pc}·]+)*$/gmu;
...
function addVar(
  ...
  type: "function" | "variable",
  view: EditorView,
  functionsNames: Set<string>,
  variablesNames: Set<string>
) {
