...
def analyze_code(input_string, level, lang):

    # Processes and parses the input to obtain relevant data
    input_string = process_input_string(input_string, level, lang)

    abstract_syntax_tree, lookup_table, commands = create_AST(input_string, level, lang)

    print(f"Abstract syntax tree: {abstract_syntax_tree}")  # Debug print

    # Occurrences of commands
    command_counter = Counter(commands)
    print(f"Command counter: {command_counter}")  # Debug print

    # Get variables from the lookup table
    variables = {entry.name for entry in lookup_table}

    print(f"Variables: {variables}")  # Debug print

    # Calculate the number of lines
    raw_lines = input_string.split('\n')
    num_lines = len(raw_lines)

    # Store the lines after stripping whitespace
    lines = [line.strip() for line in raw_lines]

    # Dictionary to store the extracted information
    info = {
        'num_lines': num_lines,
        'num_ask': command_counter['ask'],
        'num_print': command_counter['print'],
        'num_echo': command_counter['echo'],
        'num_play': command_counter['play'],
        'num_forward': command_counter['forward'],
        'num_turn': command_counter['turn'],
    ...
    print(f"Analysis info: {info}")  # Debug print

    return info


def generate_question(input_string, level, lang):
    info = analyze_code(input_string, level, lang)
    question_list = []

    def add_print_questions(line, i):
        message = line[len('print '):]  # Remove the "print " part of the line
    ...
    # Mapping command to corresponding question adding function
    command_to_function = {
        'print': add_print_questions,
        'ask': add_ask_questions,
        'echo': add_echo_questions,
        'play': add_play_questions,
        'forward': add_forward_questions,
        'turn': add_turn_questions
    }

    # Collects questions based on the commands found in the code
    for i, line in enumerate(info['lines'], 1):
        ...
        for command in command_to_function:
            if stripped_line.startswith(command):
                command_to_function[command](stripped_line, i)
...
# Example input_string because I dont know how to use the original input string. Cant figure out how back and front communicate
...
level = 1
lang = 'en'
question = generate_question(input_string, level, lang)
