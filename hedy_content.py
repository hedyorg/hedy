...
ADVENTURE_ORDER_PER_LEVEL = {
    ...
    16: ["default", "random_command", "haunted", "songs", "language"],
    17: ["default", "print_command", "elif_command", "tic", "blackjack"],
    18: ["default", "print_command", "story", "songs"],
}

RESEARCH = {}
for paper in sorted(
    os.listdir("content/research"),
...
for folder in os.listdir("translations"):
    ...
    if filter(lambda x: x.endswith(".mo"), os.listdir(locale_dir)):
        languages[folder] = CUSTOM_LANGUAGE_TRANSLATIONS.get(
            folder, static_babel_content.LANGUAGE_NAMES.get(folder, folder)
        )
...
def deep_translate_keywords(yaml, keyword_language):
    ...
    if isinstance(yaml, dict):
        return {
            k: deep_translate_keywords(v, keyword_language) for k, v in yaml.items()
        }
    return yaml
...
class Adventures(StructuredDataFile):
    ...
    def get_adventure_keyname_name_levels(self):
        return {
            aid: {adv["name"]: list(adv["levels"].keys())}
            for aid, adv in self.file.get("adventures", {}).items()
        }

    def get_adventure_names(self):
        return {
            aid: adv["name"] for aid, adv in self.file.get("adventures", {}).items()
        }
...
class ParsonsProblem(StructuredDataFile):
    ...
    def get_highest_exercise_level(self, level):
        return max(
            int(lnum) for lnum in self.file.get("levels", {}).get(level, {}).keys()
        )

    def get_parsons_data_for_level(self, level, keyword_lang="en"):
        return deep_translate_keywords(
            self.file.get("levels", {}).get(level, None), keyword_lang
        )

    def get_parsons_data_for_level_exercise(self, level, excercise, keyword_lang="en"):
        return deep_translate_keywords(
            self.file.get("levels", {}).get(level, {}).get(excercise), keyword_lang
        )
...
class Quizzes(StructuredDataFile):
    ...
    def get_quiz_data_for_level(self, level, keyword_lang="en"):
        return deep_translate_keywords(
            self.file.get("levels", {}).get(level), keyword_lang
        )

    def get_quiz_data_for_level_question(self, level, question, keyword_lang="en"):
        return deep_translate_keywords(
            self.file.get("levels", {}).get(level, {}).get(question), keyword_lang
        )
...
class Tutorials(StructuredDataFile):
    ...
    def get_tutorial_for_level_step(self, level, step, keyword_lang="en"):
        if level not in ["intro", "teacher"]:
            level = int(level)
        return deep_translate_keywords(
            self.file.get(level, {}).get("steps", {}).get(step), keyword_lang
        )
